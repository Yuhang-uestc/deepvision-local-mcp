...
├── call_tool.py               命令行工具入口（CLI 兜底，不依赖客户端注入）
├── register_mcp.py            安全注册 MCP 的 Python 实现（备份/校验/回滚）
├── check_mcp.py               MCP 连通性握手探针
├── AGENTS.md                  项目级识图规范（Codex 自动加载）
├── AGENTS.global.template.md  全局 AGENTS 模板（安装时注入本机路径）
